...
public class RouteValueRequestCultureProvider(LocalizationSettings settings) : IRequestCultureProvider
{
    ...
    public Task<ProviderCultureResult?> DetermineProviderCultureResult(HttpContext httpContext)
    {
        ...
        if (!path.HasValue)
        {
            return Task.FromResult<ProviderCultureResult?>(new ProviderCultureResult(_settings.DefaultCulture));
        }
        ...
        return Task.FromResult<ProviderCultureResult?>(new ProviderCultureResult(routeValues[0]));
    }
}
